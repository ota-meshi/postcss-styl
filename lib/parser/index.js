...
function isMixinFunction(node) {
    ...
    return isAllPropertyOrAt(node.block.nodes)
}

/**
 * Checks if the given nodes is all property or atrule.
 * @param {StylusNode[]}
 */
function isAllPropertyOrAt(nodes) {
    for (const n of nodes) {
        if (n.nodeName === "if") {
            if (!n.block || !n.block.nodes) {
                return false
            }
            if (!isAllPropertyOrAt(n.block.nodes)) {
                return false
            }
            if (n.elses.length) {
                for (const e of n.elses) {
                    if (e.nodeName === "block") {
                        if (!isAllPropertyOrAt(e.nodes)) {
                            return false
                        }
                    } else if (e.nodeName === "if") {
                        if (!isAllPropertyOrAt([e])) {
                            return false
                        }
                    } else {
                        // unknown
...
class StylusParser {
    ...
    expression(node, parent, info) {
        ...
        if (nodeName === "call" || nodeName === "binop") {
    ...
    ident(node, parent, info) {
        ...
        if (valNodeName === "function") {
            this.process(node.val, parent, info)
    ...
    atruleImpl(node, { blockFirstNode, blockNode, postfix }, parent, info) {
        ...
        let { rawBefore } = this.getRawBefore(parent, atRuleSource.start)
        if (postfix) {
            atRuleRaws.postfixBefore = parseSpaceAfter(rawBefore).after
            rawBefore = ""
        } else if (parent.postfix) {
            const parentIndex = info.parent.parentNode.nodes.indexOf(parent)
            ;({ rawBefore } = this.getRawBefore(
                {
    ...
    declImpl(node, { propStartNode }, parent) {
        const propStartIndex =
            this.sourceCode.getIndex(propStartNode) +
            (propStartNode.nodeName === "expression"
                ? -1 /* `{expression}` */
                : 0)
    ...
    getBlockEndIndex(bodyStartIndex, parent, info) {
        let bodyText = null
        ...
        return bodyStartIndex + bodyText.length - 1
    }

    getRawBefore({ last, source: parentSource }, nodeStartLoc) {
        let rawBefore = ""
        if (last) {
