...
class StylusParser {
    ...
    media(node, parent, info) {
        ...
        const beforeBlockNodeEndIndex = this.sourceCode.getEndIndex(querys)
        this.atruleImpl(
            node,
            { first: querys, beforeBlockNodeEndIndex },
            parent,
            info
        )
    }
    ...
    charset(node, parent, info) {
        const firstIndex = this.sourceCode.getIndex(node)

        const beforeBlockNodeEndIndex = this.sourceCode.getEndIndex(node.val)
        this.atruleImpl(
            node,
            { first: firstIndex, beforeBlockNodeEndIndex },
            parent,
            info
        )
    }
