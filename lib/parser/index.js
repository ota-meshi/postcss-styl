...
const debug = require("debug")("stylus-postcss:parser")
...
function getSelectorEndIndex(node, block, sourceCode) {
    const cursor = sourceCode.createTokenCursor(sourceCode.getIndex(node), {
        endLocationIndex: sourceCode.getIndex(block.nodes[0] || block) - 1,
        scope: true,
    })
    let token = cursor.next()
    if (!token) {
        return sourceCode.getIndex(block.nodes[0] || block)
    }
...
class StylusParser {
    ...
    atruleCssLiteralImpl(node, parent, _info) {
        ...
        const postCssRoot = postcss.parse(css, { from: this.input.file })
        postCssRoot.walk(n => {
            // apply offset locations
            n.source.start =
                n.source.start.line === 1
                    ? {
                          line: cssStartLoc.line,
                          column:
                              cssStartLoc.column + n.source.start.column - 1,
                      }
                    : {
                          line: n.source.start.line + cssStartLoc.line - 1,
                ...
                n.source.end =
                    n.source.end.line === 1
                        ? {
                              line: cssStartLoc.line,
                              column:
                                  cssStartLoc.column + n.source.end.column - 1,
                          }
                        : {
                              line: n.source.end.line + cssStartLoc.line - 1,
                              column: n.source.end.column,
                          }
            }
        })
