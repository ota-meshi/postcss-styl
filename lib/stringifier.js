...
module.exports = class StylusStringifier extends Stringifier {
    ...
    raw(node, own, detect) {
        ...
        // adjust indent
        if (own === "before" && isNeedAdjustIndent(node)) {
    ...
    atrule(node, semicolon) {
        ...
        if (node.raws.identifier == null && !node.function && !node.postfix) {
            ...
            if (!node.postfix && node.nodes) {
                ...
                const end = !node.omittedSemi && semicolon ? ";" : ""
                this.builder(name + params + between + body + end, node)
            } else {
                const end =
    ...
    getIndent(node) {
        ...
        if (isNeedAdjustIndent(node)) {
            const firstSibling = parent.nodes.find(n => n.type !== "comment")
        ...
        const parentIndent = this.getIndent(parent)
        const parent2Indent =
            (parent.parent && this.getIndent(parent.parent)) || ""
        ...
        return `${parentIndent}${" ".repeat(
            Math.max(parentIndent.length - parent2Indent.length, 1)
        ) || "  "}`
    }
}
