...
/**
 * Get the end index of `@css`
 *
 * @param {*} sourceCode
 * @param {*} cssStartIndex
 */
function getCssLiteralEndIndex(sourceCode, cssStartIndex) {
    const cursor = sourceCode.createScopeTokenCursor(cssStartIndex)
    let token = cursor.next()
    let lastToken = null
    while (token) {
        if (cursor.scopeLevel === 0 && lastToken && lastToken.value === "}") {
            return lastToken.range[1] - 1
        }
...
class StylusParser {
    ...
    atruleCssLiteralImpl(node, parent, _info) {
        ...
        const endIndex = getCssLiteralEndIndex(
            this.sourceCode,
            cssStartIndex - 1
        )
