...
class StylusParser {
    ...
    ternary(node, parent, info) {
        if (
            node.cond.nodeName === "binop" &&
            node.cond.op === "is defined" &&
            node.cond.left.nodeName === "ident" &&
            node.trueExpr.nodeName === "expression" &&
            node.trueExpr.nodes.length === 1 &&
            node.trueExpr.nodes[0].nodeName === "ident" &&
            node.cond.left.name === node.trueExpr.nodes[0].name &&
            node.falseExpr.nodeName === "ident" &&
            node.falseExpr.name === node.trueExpr.nodes[0].name &&
            node.falseExpr.val.nodeName === "expression" &&
            node.falseExpr.val.nodes.length === 1 &&
            node.falseExpr.val.nodes[0].nodeName === "object"
        ) {
            // conditional assignment
            const object = node.falseExpr.val.nodes[0]
            const decl = this.declImpl(
                node,
                { propStartNode: node, atblockLast: object },
                parent,
                info
            )
