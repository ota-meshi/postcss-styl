...
class SourceCode {
    ...
    getTokenIndex(startLocationIndex) {
        const map = this.tokenIndexMap
        let idx = startLocationIndex
        while (idx >= 0) {
            const i = map[idx]
            if (i != null) {
                return i
    ...
    createTokenCursor(startLocationIndex, endLocationIndex) {
        const { tokens } = this
        const startIndex = this.getTokenIndex(startLocationIndex)
        let curIndex = null
        return {
            prev() {
                const prev = curIndex == null ? startIndex : curIndex - 1
                if (prev < 0) {
                    return null
                }
                ...
                if (
                    ...
                    endLocationIndex > token.range[1]
                ) {
                    return null
                }
                curIndex = prev
            ...
            next() {
                const next = curIndex == null ? startIndex : curIndex + 1
                ...
                if (
                    endLocationIndex != null &&
                    endLocationIndex < token.range[0]
                ) {
                    return null
                }
                curIndex = next
    ...
    *genTokens(startLocationIndex, endLocationIndex) {
        const cursor = this.createTokenCursor(
            startLocationIndex,
            endLocationIndex
        )
        let token = null
    ...
    findTokenIndex(locationIndex, callback) {
        const { tokens } = this
        for (
            ...
            if (callback(tokens[index], index)) {
                return index
            }
        }
        return -1
