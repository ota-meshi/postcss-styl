...
/**
 * Check whether if given node is `!important`
 * @param {object} node node to check
 * @returns {boolean} `true` if given node is `!important`
 */
function isImportent(node) {
    if (node.nodeName === "unaryop" && node.op === "!") {
        const text = extractNodeText(node.expr)
...
/**
 * The string from segment
 * @param {stylusNode} node node
 * @returns {string} text
 */
function extractNodeText(node) {
    if (node.nodeName === "literal") {
        if (typeof node.val === "string") {
            return node.val
        }
        return extractNodeText(node.val)
    }
    ...
    if (node.nodeName === "unaryop") {
        return node.op + extractNodeText(node.expr)
    }
    if (node.nodeName === "unit") {
        return node.raw || `${node.val}`
    }
    if (node.nodeName === "string") {
        return node.quote + node.string + node.quote
    }
...
function parseSpaces(s) {
    const b = parseSpaceBefore(s)
    const a = parseSpaceAfter(b.value)
    return {
        before: b.before,
        value: a.value,
        after: a.after,
...
class StylusParser {
    ...
    process(node, parent, info) {
        if (this.sourceCode.text.includes("@Charset")) {
            debugger
        }
        ...
        if (this.sourceCode.text.includes("test08")) {
            debugger
        }
        return null
    }
    ...
    media(node, parent, info) {
        const querys = node.val
        const lastQuery = querys.nodes[querys.nodes.length - 1]
        const lastFeature = lastQuery.nodes[lastQuery.nodes.length - 1]
        const firstNode = querys
        const lastExprNode =
            lastFeature.expr.nodes[lastFeature.expr.nodes.length - 1]

        const beforeBlockNodeEndIndex =
            this.sourceCode.getIndex(lastExprNode) +
            extractNodeText(lastExprNode).length -
            1
        this.atruleImpl(
            node,
    ...
    charset(node, parent, info) {
        const firstLoc = parent.source.start

        const beforeBlockNodeEndIndex =
            this.sourceCode.getIndex(node.val) +
            extractNodeText(node.val).length -
            1
        this.atruleImpl(
            node,
            { first: firstLoc, beforeBlockNodeEndIndex },
            parent,
            info
        )
    }
    ...
    atrule(node, parent, info) {
        ...
        const beforeBlockNodeEndIndex = lastSeg
            ? this.sourceCode.getIndex(lastSeg) +
              extractNodeText(lastSeg).length -
              1
     ...
     * @param {ProcessInfo} info
     */
    atruleImpl(node, { first, beforeBlockNodeEndIndex }, parent, info) {
        ...
        if (node.block) {
            ...
            const paramsAndSpaces = parseSpaces(
                this.sourceCode.getText(
                    this.sourceCode.getIndex(first) + 1 /* @ */ + name.length,
                    blockStartIndex - 1
                )
            )
        ...
        if (node.block && node.block.nodes) {
            ...
            // raws.semicolon
            const lastAstNode = atRule.last
            if (lastAstNode && lastAstNode.type === "decl") {
    ...
    selector(node, parent, info) {
        ...
        const ruleSelector = selectors
            .map(sel => extractNodesText(sel.segments))
            // TODO normalize
            .join(",")
            .trim()
        ...
        const rawCssSelector = rawSelectorTexts
            .map(t => t.replace(/,$/u, ""))
            .join(",")
    ...
    property(node, parent, _info) {
        const valueNodes = [...node.expr.nodes]
        const actualValueNodes = valueNodes.filter(
            expr => expr.nodeName !== "comment"
        )
        let importantNode = null
        if (isImportent(actualValueNodes[actualValueNodes.length - 1])) {
            importantNode = actualValueNodes.pop()
            valueNodes.splice(valueNodes.indexOf(importantNode), 1)
        }
        const prop = extractNodesText(node.segments)
        const value = actualValueNodes
            .map(extractNodeText)
            .map(s => s.trim())
            .join(" ")

        const lastPropNode = node.segments[node.segments.length - 1]
        const lastValueNode = valueNodes[valueNodes.length - 1]
        const firstValueNode = actualValueNodes[0]

        // location
        const startIndex = this.sourceCode.getIndex(node)
        const valueEndIndex =
            this.sourceCode.getIndex(lastValueNode) +
            extractNodeText(lastValueNode).length -
            1
        let endIndex = valueEndIndex
        const beforeSemi = /^\s*;/u.exec(this.sourceCode.getText(endIndex + 1))
        ...
        // raws
        const afterPropIndex =
            this.sourceCode.getIndex(lastPropNode) +
            extractNodeText(lastPropNode).length
        const rawBetweenStylus = this.sourceCode.getText(
            afterPropIndex,
            this.sourceCode.getIndex(firstValueNode) - 1
        )
        const rawCssValue = extractNodesText(valueNodes)
        const rawStylusValue = this.sourceCode.getText(
            firstValueNode,
            importantNode
                ? importantNode
                : hasSemi
        ...
        if (importantNode && extractNodeText(importantNode) !== "!important") {
            declRaws.important = extractNodeText(importantNode)
        }
